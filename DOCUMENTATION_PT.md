...
- Suporte a `auto_commit` para gerenciamento do ciclo transacional.
...
## Ferramenta CLI de Inicialização (`cli.py`)

Para sementação e provisionamento seguro do banco de dados, implementamos um utilitário CLI interativo em [cli.py](file:///e:/GitHub/fast-api-min/cli.py):
- **`createsuperuser`**: Solicita credenciais interativas (e-mail, nome, senha) com validações e checagem de duplicidade no banco.
- **`seed-permissions`**: Varre os modelos cadastrados e semeia as permissões CRUD básicas no banco.

---

## Endpoint de Diagnóstico (Health Check)

O endpoint `/health` em [health.py](file:///e:/GitHub/fast-api-min/app/api/v1/health.py) testa o liveness de componentes principais:
1. Banco de dados **PostgreSQL** através de `SELECT 1`.
2. Servidor de Cache **Redis** através de `ping()`.

Caso qualquer serviço esteja off-line, o app retorna `503 Service Unavailable` com o mapeamento do erro.
